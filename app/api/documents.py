...
from app.services.document_generator import generate_full_packet
...
def _generate_and_save(session_id: str, data: dict):
    """Background task — creates its OWN db session, never reuses request session."""
    db = SessionLocal()
    try:
        ...
        print(f"[GENERATION ERROR] session={session_id}\n{tb}")
        # Mark session as error so frontend stops polling
        try:
            session = db.query(Session).filter(Session.id == session_id).first()
            if session:
                session.stage = "error"
            ...
            print(f"[GENERATION ERROR] Failed to save error state: {inner}")
    finally:
        db.close()
...
async def generate_packet(
    ...
    # Pass only session_id + data — NOT the db session
...
def packet_status(session_id: str, db: DBSession = Depends(get_db)):
    ...
    return {
        ...
        "error": getattr(session, "error_message", None) if session.stage == "error" else None
    }
...
def download_packet(session_id: str, db: DBSession = Depends(get_db)):
    ...
    name = (session.data or {}).get("petitioner_full_name", "user").replace(" ", "_")
    return FileResponse(
